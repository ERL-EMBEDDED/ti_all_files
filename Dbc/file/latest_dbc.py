...
def encodeCANMessage(decoded):
    ...
    wheelSpeedFR_encoded = int((decoded.wheelSpeedFR - 1.0) / _config_wheelSpeedFR_Scale)
    wheelSpeedFL_encoded = int((decoded.wheelSpeedFL - 1.0) / _config_wheelSpeedFL_Scale)
...
def parse_can_message(hex_string):
    ...
    # Extract CAN ID (assuming first 3 characters) and data (the rest of the string)
        ...
        if ser.isOpen():
            ...
            if received_data:
                ...
                try:
                    ...
                    # Encode the decoded message with -1.0 subtraction
                    encoded_message = encodeCANMessage(decoded_message)
                    log_data("Encoded Msg", encoded_message)
                    ...
                    Actual_Wheel_Speed_FR = Expected_Wheel_Speed_FR - 1.0
                    Actual_Wheel_Speed_FL = Expected_Wheel_Speed_FL - 1.0
                    Actual_Vehicle_Speed  = Expected_Vehicle_Speed - 1.0
                    ...
                    # Log expected values (subtracting 1.0)
                    log_data("Actual_Wheel Speed FR", Actual_Wheel_Speed_FR)
                    log_data("Actual_Wheel Speed FL", Actual_Wheel_Speed_FL)
                    log_data("Actual_Vehicle Speed", Actual_Vehicle_Speed)
