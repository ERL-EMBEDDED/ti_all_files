    ...
    while (std::getline(file, line)) {
        ...
        try {
            if (line.find("BO_") == 0) { // Message definition
                ...
                std::cout << "Parsed Message: ID = " << id << ", Name = " << name << ", Length = " << length << std::endl;
                ...
                if (separator != std::string::npos) {
                    ...
                    iss >> byteOrderSigned;
                    signal.byteOrder = byteOrderSigned[0] == '0' ? "Motorola" : "Intel";
                    signal.isSigned = byteOrderSigned[1] == '-';

                    std::string scaleOffset;
                    iss >> scaleOffset;
                    size_t scaleSep = scaleOffset.find(',');
                    signal.scale = std::stod(scaleOffset.substr(0, scaleSep));
                    signal.offset = std::stod(scaleOffset.substr(scaleSep + 1));

                    std::string minMax;
                    iss >> minMax;
                    size_t minMaxSep = minMax.find('|');
                    signal.minimum = std::stod(minMax.substr(1, minMaxSep - 1)); // Remove '['
                    signal.maximum = std::stod(minMax.substr(minMaxSep + 1, minMax.length() - minMaxSep - 2)); // Remove ']'
                    ...
                    // Debugging: Print the signal details after parsing
                    std::cout << "Parsed Signal: " << signal.name << " Start Bit: " << signal.startBit 
                              << " Length: " << signal.length << " Byte Order: " << signal.byteOrder 
                              << " Signed: " << (signal.isSigned ? "True" : "False") << " Scale: " << signal.scale 
                              << " Offset: " << signal.offset << " Minimum: " << signal.minimum 
                              << " Maximum: " << signal.maximum << " Unit: " << signal.unit << std::endl;
