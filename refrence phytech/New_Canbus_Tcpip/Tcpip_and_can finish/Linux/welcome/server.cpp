...
int main()
{
    ...
    serverAddr.sin_addr.s_addr = inet_addr("192.168.3.12"); // Set server IP address
    ...
    while (true)
    {
        ...
        // Read input from cin and send it to the client
        std::string inputData;
        std::cout << "Enter data to send to the client: ";
        std::getline(std::cin, inputData);

        // Populate the block with input data
        Block block;
        block.Amount = inputData.size();
        if (inputData.size() > IPC_BLOCK_SIZE) {
            std::cerr << "Data size exceeds block size\n";
            close(clientSocket);
            continue;
        }
        std::copy(inputData.begin(), inputData.end(), block.Data);
