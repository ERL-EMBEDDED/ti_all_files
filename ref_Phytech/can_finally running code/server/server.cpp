...
int epsServer::receiveMessage() {
	...
	if (startPos != std::string::npos) {
		...
		if (endPos != std::string::npos) {
			...
			try {
			       	can.sendEncodedData(std::stoi(speedStr));
				std::cout << "Speed: " << *expectedVehicleSpeed<< std::endl;
			} catch (...) {
...
int epsServer::sendMessage(float speed) {
	std::cout << "actualVehicleSpeed : " << speed<<std::endl;
	std::string send_Message = "{\"Speed\":" + std::to_string(speed) + "}";
	...
	std::cout << "Message sent successfully: " << send_Message << std::endl;
	return bytesSent;
}
