...
void CanInterface::ReceivedData() {
    ...
    // Reverse endian and reconstruct the convo structure
    ...
    uint64_t endian_conv = boost::endian::endian_reverse(big_endian);
    std::cout << "endian conv value: " << std::hex << endian_conv << std::endl;


    // Copy the reversed value back to convo.values
    std::memcpy(convo.values, &endian_conv, sizeof(convo.values));

    uint16_t receivedValue = (convo.values[2] << 8) | convo.values[1];
    int receivedValue_dec = static_cast<int>(receivedValue);

    // Print all the extracted values for debugging
    std::cout << "Received Values: " << std::endl;
    std::cout << "Wheel Speed FR: " << ((convo.values[6] << 8) | convo.values[5]) << std::endl;
    std::cout << "Wheel Speed FL: " << ((convo.values[4] << 8) | convo.values[3]) << std::endl;
    std::cout << "Vehicle Speed: " << receivedValue << std::endl;
    std::cout << "Vehicle Speed Clock: " << convo.values[0] << std::endl;

    std::cout << "Vehicle Speed Hexadecimal: " <<  receivedValue << ",Vehile Speed Decimal: " << receivedValue_dec << std::endl;
}
...
void CanInterface::sendEncodedData() {
    ...
    _by_convo_value = speed;    // Vehicle Speed
